    ...
    protected function toolbar_wpda()
    {
        ...
        ?>
				...
				<div class="wpda-promotion" style="font-size: 16px">
					<?php 
        $current_version = get_user_meta( WPDA::get_current_user_id(), 'wpda_data_explorer', true );
        
        if ( 'new' !== $current_version ) {
            ?>
						<a href="?page=<?php 
            echo  esc_attr( WP_Data_Access_Admin::PAGE_MAIN ) ;
            ?>&de=new"><i class="fas fa-toggle-off"></i> Switch to new Data Explorer</a>
						<?php 
        } else {
            ?>
						<a href="?page=<?php 
            echo  esc_attr( WP_Data_Access_Admin::PAGE_MAIN ) ;
            ?>&de=old"><i class="fas fa-toggle-on"></i> Switch to old Data Explorer</a>
						<?php 
        }
