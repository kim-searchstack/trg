...
namespace WPDataAccess\API {
	...
	class WPDA_Settings {
		...
		public static function save_explorer_settings( $schema_name, $table_name, $settings ) {
			global $wpdb;
			$dml_failed = 0;
			...
			if ( is_array( $media_columns ) ) {
				foreach ( $media_columns as $column => $value ) {
					if ( false === $value || '' === $value ) {
						...
						if ( 1 !== WPDA_Media_Model::update( $table_name, $column, $schema_name ) ) {
			...
			if ( 0 === $dml_failed ) {
				return WPDA_API::WPDA_Rest_Response( __( 'Changes successfully saved', 'wp-data-access' ) );
			} else {
				if ( '' === $wpdb->last_error ) {
					return new \WP_Error(
		...
		public static function save_admin_settings( $schema_name, $table_name, $settings ) {
			...
			$admin_settings = self::get_admin_settings_key(
				$settings['target'],
				$schema_name ,
				$table_name
			);
			...
			return WPDA_API::WPDA_Rest_Response( __( 'Successfully saved changes', 'wp-data-access' ) );
		}
